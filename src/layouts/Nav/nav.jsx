import {Container, Navbar, Nav} from 'react-bootstrap'
import classes from './nav.module.css'

export const NavHeader =()=>{

    // displays social media handles

    return (
        <Navbar bg="light" expand="lg" className={classes.wrapper}>
            <Container className={classes.container} >
                <Nav className={classes.logo}>
                    <Navbar.Brand href="#home">METRICKS</Navbar.Brand>
                </Nav>
                <Navbar.Collapse id="basic-navbar-nav" className={classes.navHolder}>
                    <Nav className={classes.nav}>
                        <Nav.Link href="#home"  className={classes.link}>ABOUT US</Nav.Link>
                        <Nav.Link href="#link"  className={classes.link}>BLOG</Nav.Link>
                        <Nav.Link href="#link"  className={classes.link} id ={classes.contact}>CONTACT US</Nav.Link>
                    </Nav>
                </Navbar.Collapse>
            </Container>
        </Navbar>
    )
}